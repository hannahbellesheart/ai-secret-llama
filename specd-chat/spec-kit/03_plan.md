...
- **Constraints**: Use only vanilla JavaScript, HTML, CSS; no Node.js dependencies; only CDNs for external resources (e.g., Bootstrap); no build tools; pure browser-based execution.
...
- **Technology Constraints**: Strictly adhere to vanilla JS, HTML, CSS only; use CDNs for external resources; no Node.js, no build tools, no ES modules beyond browser support; ensure all code runs natively in the browser.
